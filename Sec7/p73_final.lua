...
local function test_file_size_limit()
    ...
    for _, size in ipairs(sizes) do
        ...
        -- 尝试一次性读取
        local success, time, file_size, mem_used = pcall(read_entire_file, test_file)
        if success then
...
local function main()
    ...
    -- 运行性能测试
    local perf_results = run_performance_test()
    
    -- 测试文件大小限制
    local max_size, size_results = test_file_size_limit()
    ...
    print("\n4. 实际测试结果分析:")
    print("   - 按字节读取: 性能最差，随文件大小线性增长")
    print("   - 按行读取: 适合文本文件，性能中等")
    print("   - 按块读取: 性能最好，内存使用稳定")
    print("   - 一次性读取: 性能好但受内存限制")
    
    print("\n测试完成！")
end

-- 如果直接运行此脚本，则执行主程序
if arg[0]:match("p73_final%.lua$") then
    main()
end 